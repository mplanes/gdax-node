...
class OrderbookSync extends WebsocketClient {
  constructor(
    ...
    super(productIDs, websocketURI, auth, { heartbeat, channels });
    this.apiURI = apiURI;
  ...
  loadOrderbook(productID) {
    ...
    if (this.authenticatedClient) {
      ...
      this._public_clients[productID]
        ...
        .then(onData.bind(this))
        .catch(onError.bind(this));
      this.emit('bookupdate', productID, new Date().toISOString());
    }
  ...
  processMessage(data) {
    ...
    if (data.type == 'heartbeat') {
      this.emit('heartbeat', data.product_id, data.time);
      return
    }
    ...
    switch (data.type) {
      ...
      default:
        this.emit('error', 'unknown update type: ' + data.type);
    }
    this.emit('bookupdate', product_id, data.time);
  }
}
